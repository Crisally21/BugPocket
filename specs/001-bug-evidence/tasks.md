...
Обновлено 2026-09-25 после решения Q-04 и проверки размера файла. **T-01 — DONE; T-05 — IN PROGRESS**. Код T-02–T-04 уже присутствует в проекте; их полная приёмка не подтверждается этим обновлением. T-06–T-12 остаются TODO.
Это обновление фиксирует решение о байтовой границе и фактически проверенную часть T-05; оно не означает готовность загрузки вложений через API или UI.

## Принятые решения и оставшиеся вопросы

- **Q-04 закрыт (2026-09-25):** 25 MB = 25 000 000 байт для одного файла и суммы вложений бага. Равенство разрешено; превышение хотя бы на байт отклоняется. Вопрос больше не блокирует задачи.
...
Attachment API отделён от JSON POST/PUT bug. Существующий api() в app.js требует поддержки FormData и пустых ответов; выбранные File не должны попадать в JSON DTO. Изоляция файловых тестов обязательна; в BugApiTests и RelatedTaskUrlApiTests вложения уже удаляются перед багами. Публичные metadata не раскрывают storageKey/пути.

## Задачи

### T-01 — Related Task URL: backend и БД

- **Статус:** DONE (2026-09-23). Реализованы backend/БД и тесты T-01. Команда: `JAVA_HOME=C:\Program Files\Java\jdk-17` (переменная PowerShell), затем `.\mvnw.cmd -B -ntp test`: 34 теста, 0 ошибок, 0 пропусков; включая BugApiTests, contextLoads, URL API и upgrade Liquibase на H2.
...
- **Статус:** IN PROGRESS (2026-09-25). В AttachmentValidator реализованы validateNotEmpty и validateSize: null и пустой файл отклоняются, пофайловый лимит — 25 000 000 байт включительно. В AttachmentFileType добавлены категория медиа, MIME-тип и разбор расширения/имени; это не проверка фактического содержимого.
- **Выполненная проверка:** `.\mvnw.cmd -B -ntp '-Dtest=AttachmentValidatorTests' test` с JDK 17 — 5 тестов, 0 ошибок, 0 пропусков: B−1, B, B+1, пустой файл и null. Используется MockMultipartFile с массивами байтов, а не валидными медиа. Проверки границ на настоящих медиа, содержимого и видеопревью ещё предстоят. Полный набор тестов после добавления этих пяти тестов пока не запускался.
...
- **Зависимости:** T-04. **Частично BLOCKED:** Q-06 для результата preview failure (Q-04 закрыт); выбор библиотеки и проверка runtime — часть этой задачи.
...
- **Зависимости:** T-03–05. **Частично BLOCKED:** Q-09/Q-06 для соответствующих правил. Q-04 закрыт: B = 25 000 000 байт.
...
- **Зависимости:** T-08; Q-09/Q-06 должны быть закрыты для соответствующей приёмки. Q-04 закрыт.
...
- **Зависимости:** T-01–10; для полной приёмки должны быть получены решения Q-09/Q-06. Q-04 уже закрыт. Пропущенную инфраструктурную проверку не считать пройденной.
